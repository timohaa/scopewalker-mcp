...
| Ruby                  | `class`                    | -                               | -      | top-level and module-body `def`               | -                         |
...
- Class methods are nested under their class and omitted from the top-level function list. Go methods match their type by receiver across every file in the package. A type in `types.go` can therefore collect methods from `methods.go`. Matching stays within each directory, which separates same-named types in different packages. C/C++ member functions are picked up from the record body, including declaration-only members. Constructors, destructors, and operator overloads count as members. A member defined outside its class body is not listed again at the top level when the class is declared in the same file; when it is not, the definition keeps its qualified name (`Widget::size`). A Rust trait's members, both required signatures and default methods, are listed under the trait. Rust `impl` methods are currently reported as standalone functions.
- Go grouped `type (...)` declarations produce one item per spec at its own line. Each spec or alias maps to a class when its child is a struct; other shapes map to interfaces.
- Ruby module methods appear as top-level functions, including definitions wrapped in conditionals. There is no module item to contain them. Class members stay nested under their class; methods inside `class << self` remain omitted. Module functions use the same underscore-based private filtering as other Ruby functions.
- `summary.total_methods` counts methods listed under both classes and interfaces. Interfaces do not contribute to `summary.total_classes`.
