...
Comments on type/interface fields, enum members, config constants, and similar declarations document **domain meaning**, not implementation. A comment like `isAdminGift?: boolean; // gifted by an admin` is valuable documentation even though it reads as "what" — the redundancy rule does NOT apply to it.
