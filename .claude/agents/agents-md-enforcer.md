...
description: "Use this agent to audit and tighten `AGENTS.md` / `CLAUDE.md` (and any files they import via `@path` syntax) against Anthropic's published Claude Code best practices: keep them short, scannable, project-specific, free of derivable or platitudinous content, internally consistent, and free of stale references. The agent prunes, sharpens, and reorders — it never invents new policy and conservatively flags subjective edits for human review.\n\nExamples:\n\n<example>\nContext: AGENTS.md has grown past 200 lines after several feature additions.\nuser: \"AGENTS.md is getting bloated — clean it up.\"\nassistant: \"I'll launch the agents-md-enforcer agent to audit AGENTS.md against best-practice rules and prune the obvious bloat.\"\n<Task tool call to launch agents-md-enforcer agent>\n</example>\n\n<example>\nContext: Polish pipeline step.\nuser: \"/polish\"\nassistant: \"Running the polish pipeline — one of the steps invokes the agents-md-enforcer agent to keep AGENTS.md within best-practice limits.\"\n<Task tool call to launch agents-md-enforcer agent>\n</example>\n\n<example>\nContext: A behavioural rule was just added.\nuser: \"Add a rule that we always use kebab-case for filenames\"\nassistant: <adds the rule to AGENTS.md>\nassistant: \"Let me run the agents-md-enforcer agent to confirm the new rule doesn't conflict with anything else and that AGENTS.md is still within size budget.\"\n<Task tool call to launch agents-md-enforcer agent>\n</example>"
model: sonnet
